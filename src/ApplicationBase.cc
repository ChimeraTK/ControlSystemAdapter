...
ApplicationBase::ApplicationBase(const std::string &name)
  ...
  if (instance != nullptr) {
    throw ChimeraTK::logic_error(
        "Multiple instances of ChimeraTK::ApplicationBase cannot be created.");
  }
  instance = this;
}
...
ApplicationBase::~ApplicationBase() {
  if (!hasBeenShutdown) {
    std::cerr << "*************************************************************"
                 "****************"
              << std::endl;
    std::cerr << " BUG found in application " << applicationName << "!"
              << std::endl;
...
void ApplicationBase::setPVManager(
    boost::shared_ptr<ChimeraTK::DevicePVManager> const
...
ApplicationBase &ApplicationBase::getInstance() {
  if (instance == nullptr) {
    throw ChimeraTK::logic_error(
        "No instance of ChimeraTK::ApplicationBase created, but "
