...
void PersistentDataStorage::writeToFile() {
  ...
  // root
  xmlpp::Document doc;
  xmlpp::Element *rootElement = doc.create_root_node(
      "PersistentData", "https://github.com/ChimeraTK/ControlSystemAdapter");
  ...
  for (size_t i = 0; i < _variableNames.size(); ++i) {
    ...
    variable->set_attribute("name",
                            static_cast<std::string>(_variableNames[i]));
...
void PersistentDataStorage::generateXmlValueTags(xmlpp::Element *parent,
                                                 size_t id) {

  // obtain the data vector from the map
  std::vector<DataType> &value =
      boost::fusion::at_key<DataType>(_dataMap.table)[id];
  ...
  for (size_t idx = 0; idx < value.size(); ++idx) {
    ...
    valueElement->set_attribute("v",
                                boost::lexical_cast<std::string>(value[idx]));
  }
}
...
void PersistentDataStorage::readFromFile() {
  ...
  if (stat(_filename.c_str(), &buffer) != 0) {
    ...
    std::cerr << "ChimeraTK::PersistentDataStorage: Persistency file '"
              << _filename
              << "' does not exist. "
  ...
  try {
    ...
    if (parser) {
      ...
      for (auto &elem : rootElement->get_children()) {
        ...
        if (type == "int8") {
          readXmlValueTags<int8_t>(child,
                                   registerVariable<int8_t>(name, 0, true));
        } else if (type == "uint8") {
          readXmlValueTags<uint8_t>(child,
                                    registerVariable<uint8_t>(name, 0, true));
        } else if (type == "int16") {
          readXmlValueTags<int16_t>(child,
                                    registerVariable<int16_t>(name, 0, true));
        } else if (type == "uint16") {
          readXmlValueTags<uint16_t>(child,
                                     registerVariable<uint16_t>(name, 0, true));
        } else if (type == "int32") {
          readXmlValueTags<int32_t>(child,
                                    registerVariable<int32_t>(name, 0, true));
        } else if (type == "uint32") {
          readXmlValueTags<uint32_t>(child,
                                     registerVariable<uint32_t>(name, 0, true));
        } else if (type == "float") {
          readXmlValueTags<float>(child,
                                  registerVariable<float>(name, 0, true));
        } else if (type == "double") {
          readXmlValueTags<double>(child,
                                   registerVariable<double>(name, 0, true));
        } else if (type == "string") {
          readXmlValueTags<std::string>(
              child, registerVariable<std::string>(name, 0, true));
        } else { /* @todo TODO ??? */
        }
      }
    } else {
      std::cout << "No parser... "
                << std::endl; // @todo TODO proper exception handling
    }
...
template <typename DataType>
void PersistentDataStorage::readXmlValueTags(const xmlpp::Element *parent,
                                             size_t id) {
  ...
  for (auto &valElems : parent->get_children()) {
    const xmlpp::Element *valChild =
        dynamic_cast<const xmlpp::Element *>(valElems);
    if (!valChild)
      continue; // comment or white spaces...
