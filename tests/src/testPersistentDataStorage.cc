...
// define empty test application to fullfill the requirement of having an
// instance
...
/**
 *  @todo FIXME No exception tests are yet done. They should be implemented
 * asap!
 */
/*

// Test storing data in first instance of PersistentDataStorage and retrieveing
it in a second instance BOOST_AUTO_TEST_CASE( testStoreAndRetrieve ) {

    // remove persistency file from previous test run
    boost::filesystem::remove("myTestApplication.persist");

    // create PersistentDataStorage for the first time and fill it with two
variables
    {
      ...
      // register floating-point variable /some/path.with.dots/to/MyVar2 with
100 array elements and fill it std::vector<double> myVar2(100); int id2 =
storage.registerVariable<double>("/some/path.with.dots/to/MyVar2", 100); for(int
i=0; i<100; ++i) myVar2[i] = -120 + 7*i; storage.updateValue(id2, myVar2);
      ...
      for(int i=0; i<100; ++i) BOOST_CHECK_CLOSE( myVar2stored[i], -120 + 7*i,
0.0001 );
    }
    // the first PersistentDataStorage is destroyed at this point, in its
destructor the file is written

    // create another PersistentDataStorage and read the previously stored
variables from it
    {
      PersistentDataStorage storage{"myTestApplication"};
      ...
      int id2 =
storage.registerVariable<double>("/some/path.with.dots/to/MyVar2", 100); myVar2
= storage.retrieveValue<double>(id2); for(int i=0; i<100; ++i)
...
BOOST_AUTO_TEST_CASE( testUsageInPVManager ) {
    ...
    // first application instance: initialise the variables with some values and
store them in the persistency file
    {
...
"SomeCsToDevVar", 7);
      devManager->createProcessArray<float>(SynchronizationDirection::controlSystemToDevice,
"AnotherCsToDevVar", 42);
      devManager->createProcessArray<int32_t>(SynchronizationDirection::deviceToControlSystem,
"SomeDevToCsVar", 7);
      ...
      auto v3 = devManager->getProcessArray<int32_t>("SomeDevToCsVar"); // this
one won't get stored for(int i=0; i<7; ++i) v3->accessChannel(0)[i] = 9*i + 666;
      v3->write();

    }

    // second application instance: check if stored values are properly
retrieved
    {
...
persistent data storage csManager->getAllProcessVariables();
      ...
      v2->readNonBlocking();
      for(int i=0; i<42; ++i) BOOST_CHECK_CLOSE(v2->accessChannel(0)[i],
i*3.1415 * 1e30, 2e23);

      auto v3 = csManager->getProcessArray<int32_t>("SomeDevToCsVar"); // this
one won't get stored v3->readNonBlocking(); for(int i=0; i<7; ++i) BOOST_CHECK(
v3->accessChannel(0)[i] == 0 );
...
BOOST_AUTO_TEST_CASE(testChangedVariableHousehold) {
  ...
  // check if stored values are properly retrieved
  {
    ...
    devManager->createProcessArray<uint16_t>(
        SynchronizationDirection::controlSystemToDevice, "SomeCsToDevVar", 7);
    devManager->createProcessArray<float>(
        SynchronizationDirection::controlSystemToDevice, "AnotherCsToDevVar",
        42);
    devManager->createProcessArray<int32_t>(
        SynchronizationDirection::deviceToControlSystem, "SomeDevToCsVar", 7);
    ...
    auto v3 = csManager->getProcessArray<int32_t>(
        "SomeDevToCsVar"); // this one won't get stored
    v3->readNonBlocking();
    for (int i = 0; i < 7; ++i)
  ...
  BOOST_CHECK_EQUAL(countLinesInFile("changedType.persist"),
                    countLinesInFile("myTestApplication.persist"));

  // check with changed vector size
  boost::filesystem::remove("myTestApplication.persist");
  boost::filesystem::copy("changedVectorSize.persist",
  ...
  BOOST_CHECK_EQUAL(countLinesInFile("changedVectorSize.persist") + 2,
                    countLinesInFile("myTestApplication.persist"));

  // check with renamed variable
  boost::filesystem::remove("myTestApplication.persist");
  boost::filesystem::copy("renamedVariable.persist",
                          "myTestApplication.persist");
  ...
  // check that the number of lines in the file didn't change
  BOOST_CHECK_EQUAL(countLinesInFile("renamedVariable.persist"),
                    countLinesInFile("myTestApplication.persist"));
}
