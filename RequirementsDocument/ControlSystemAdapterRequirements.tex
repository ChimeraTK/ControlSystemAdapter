...
Usually all input variables are synchronied at the beginning of the 
...
\fixme{It seems like get function and implicit conversion operator have to make a copy
  because it a priory cannot be guaranteed that the controll system type allows
...
\item swap() \fixme{Do we need it ? Might not be possible to implement this efficiently.}
...
synchroinised with the control system:
\begin{enumerate}
  ...
  \item Synchronisation functions which are called within the user code.
    ...
    has to dedermine when the synchronisation is executed.
\end{enumerate}
...
seems multiple factories might be posible, and the name of the location has to be given to the
...
It is foreseen two have two synchrionisation points: One for inbound changes
...
\texttt{void synchonise1()} and \texttt{void synchonise2()} functions of the
...
Although D\_\textit{xxx}array seems like the obvious choise, it is almost never used in practice.
