...
public:
  ...
  size_t registerVariable(ChimeraTK::RegisterPath const &name, size_t nElements,
                          bool fromFile = false);
...
void PersistentDataStorage::registerVariable_oldTypeRemover::
...
template <typename DataType>
size_t
PersistentDataStorage::registerVariable(ChimeraTK::RegisterPath const &name,
                                        size_t nElements, bool fromFile) {
  ...
  if (position == _variableNames.end()) {
    // output information
    if (!fromFile)
      std::cout << "PersistentDataStorage: registering new variable " << name
                << std::endl;
  ...
  // replace element (changed data type)
    ...
    std::cout << "PersistentDataStorage: changing type of variable " << name
              << std::endl;
    assert(_variableTypes[id] != &typeid(DataType));
    assert(!fromFile);

    // remove value vector from old type
    boost::fusion::for_each(_dataMap.table, registerVariable_oldTypeRemover(
                                                {_variableTypes[id], id}));
  ...
  // return existing id
    ...
    if (value.size() != nElements) {
      std::cout << "PersistentDataStorage: changing size of variable " << name
                << std::endl;
      value.resize(nElements);
    }
...
template <typename DataType>
void PersistentDataStorage::updateValue(int id,
                                        std::vector<DataType> const &value) {
