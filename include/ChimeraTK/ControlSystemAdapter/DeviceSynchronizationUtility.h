...
namespace ChimeraTK {
  ...
  template<class T>
  void DeviceSynchronizationUtility::receive(T processVariablesBegin, T processVariablesEnd) {
  ...
  template<class T>
  void DeviceSynchronizationUtility::send(T processVariables) {
    send(processVariables.begin(), processVariables.end());
  }

  template<class T>
  void DeviceSynchronizationUtility::send(T processVariablesBegin, T processVariablesEnd) {
    for(T i = processVariablesBegin; i != processVariablesEnd; ++i) {
      (*i)->write();
    }
  }
