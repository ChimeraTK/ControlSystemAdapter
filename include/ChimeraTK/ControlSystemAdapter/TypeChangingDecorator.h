...
getDecorator(ChimeraTK::TransferElementAbstractor &transferElement,
             DecoratorType decoratorType = DecoratorType::range_checking) {
  return getDecorator<UserType>(transferElement.getHighLevelImplElement(),
                                decoratorType);
}
...
class TypeChangingDecorator
    : public ChimeraTK::NDRegisterAccessorDecorator<T, IMPL_T>,
      public DecoratorTypeHolder {

public:
  TypeChangingDecorator(
      boost::shared_ptr<ChimeraTK::NDRegisterAccessor<IMPL_T>> const
          &target) noexcept;
  ...
  bool mayReplaceOther(const boost::shared_ptr<ChimeraTK::TransferElement const>
    ...
    auto casted =
        boost::dynamic_pointer_cast<TypeChangingDecorator<T, IMPL_T> const>(
            other);
    if (!casted)
      return false;
...
class TypeChangingStringImplDecorator
    : public TypeChangingDecorator<T, IMPL_T> {
public:
...
template <class T> T stringToT(std::string const &input) {
  std::stringstream s;
  s << input;
  T t;
  s >> t;
...
template <class T> std::string T_ToString(T input) {
  std::stringstream s;
  s << input;
  std::string output;
  s >> output;
...
class TypeChangingStringImplDecorator<T, std::string>
    : public TypeChangingDecorator<T, std::string> {
public:
  ...
  void convertAndCopyFromImpl() override {
    for (size_t i = 0; i < this->buffer_2D.size(); ++i) {
      for (size_t j = 0; j < this->buffer_2D[i].size(); ++j) {
        this->buffer_2D[i][j] =
            csa_helpers::stringToT<T>(this->_target->accessChannel(i)[j]);
      }
    }
  }
    ...
    for (size_t i = 0; i < this->buffer_2D.size(); ++i) {
      for (size_t j = 0; j < this->buffer_2D[i].size(); ++j) {
        this->_target->accessChannel(i)[j] =
            csa_helpers::T_ToString(this->buffer_2D[i][j]);
      }
    }
  }
};
...
class TypeChangingRangeCheckingDecorator
    : public TypeChangingStringImplDecorator<T, IMPL_T> {
public:
  using TypeChangingStringImplDecorator<
      T, IMPL_T>::TypeChangingStringImplDecorator;
  void convertAndCopyFromImpl() override;
  void convertAndCopyToImpl() override;
...
private:
  ...
  struct BadNumericCastException : public BOOST_EXCEPTION_T {
    BadNumericCastException(std::string variableName)
        : errorMessage("Exception during type changing conversion in " +
                       variableName + ": " + BOOST_EXCEPTION_T().what()) {}
...
class TypeChangingRangeCheckingDecorator<T, std::string>
    : public TypeChangingStringImplDecorator<T, std::string> {
public:
  using TypeChangingStringImplDecorator<
      T, std::string>::TypeChangingStringImplDecorator;
  void convertAndCopyFromImpl() override {
...
void TypeChangingRangeCheckingDecorator<T, IMPL_T>::convertAndCopyFromImpl() {
  typedef boost::numeric::converter<
      T, IMPL_T, boost::numeric::conversion_traits<T, IMPL_T>,
  ...
  try {
    for (size_t i = 0; i < this->buffer_2D.size(); ++i) {
      for (size_t j = 0; j < this->buffer_2D[i].size(); ++j) {
        this->buffer_2D[i][j] =
            FromImplConverter::convert(_target->accessChannel(i)[j]);
      }
    }
  } catch (boost::numeric::positive_overflow &) {
    throw(BadNumericCastException<boost::numeric::positive_overflow>(
        this->getName()));
  } catch (boost::numeric::negative_overflow &) {
    throw(BadNumericCastException<boost::numeric::negative_overflow>(
        this->getName()));
  } catch (boost::numeric::bad_numeric_cast &boostException) {
    throw(BadNumericCastException<boost::numeric::bad_numeric_cast>(
        this->getName()));
  }
}
...
void TypeChangingRangeCheckingDecorator<T, IMPL_T>::convertAndCopyToImpl() {
  typedef boost::numeric::converter<
      IMPL_T, T, boost::numeric::conversion_traits<IMPL_T, T>,
      boost::numeric::def_overflow_handler, csa_helpers::Round<double>>
      ToImplConverter;
  for (size_t i = 0; i < this->buffer_2D.size(); ++i) {
    for (size_t j = 0; j < this->buffer_2D[i].size(); ++j) {
      _target->accessChannel(i)[j] =
          ToImplConverter::convert(this->buffer_2D[i][j]);
    }
  }
}
...
template <class UserType> class DecoratorFactory {
public:
  DecoratorFactory(boost::shared_ptr<ChimeraTK::TransferElement> theImpl_,
                   DecoratorType wantedDecoratorType_)
  ...
  mutable boost::shared_ptr<ChimeraTK::NDRegisterAccessor<UserType>>
      createdDecorator;
  ...
  template <typename PAIR> void operator()(PAIR &) const {
    ...
    if (typeid(TargetImplType) != theImpl->getValueType())
      return;

    if (wantedDecoratorType == DecoratorType::range_checking) {
      createdDecorator.reset(
          new TypeChangingRangeCheckingDecorator<UserType, TargetImplType>(
              boost::dynamic_pointer_cast<
                  ChimeraTK::NDRegisterAccessor<TargetImplType>>(theImpl)));
    } else if (wantedDecoratorType == DecoratorType::C_style_conversion) {
      createdDecorator.reset(
          new TypeChangingDirectCastDecorator<UserType, TargetImplType>(
              boost::dynamic_pointer_cast<
                  ChimeraTK::NDRegisterAccessor<TargetImplType>>(theImpl)));
    } else {
...
boost::shared_ptr<ChimeraTK::NDRegisterAccessor<UserType>> getDecorator(
  ...
  if (decoratorMapEntry != getGlobalDecoratorMap().end()) {
    ...
    auto castedType =
        boost::dynamic_pointer_cast<ChimeraTK::NDRegisterAccessor<UserType>>(
            decoratorMapEntry->second);
    if (castedType) { // User type matches,  but the decorator type also has to
                      // match
      auto decoTypeHolder = boost::dynamic_pointer_cast<DecoratorTypeHolder>(
          decoratorMapEntry->second); /// @todo eliminate this cast and change
                                      /// map to hold DecoratorTypeHolder
      ...
      if (decoTypeHolder->getDecoratorType() == decoratorType) {
        ...
        throw ChimeraTK::logic_error(
            "ChimeraTK::ControlSystemAdapter: Decorator for TransferElement " +
            transferElement->getName() +
            " already exists as a different decorator type.");
      }
    } else {
      // sorry there is a decorator, but it's the wrong user type
      throw ChimeraTK::logic_error(
          "ChimeraTK::ControlSystemAdapter: Decorator for TransferElement " +
          transferElement->getName() +
          " already exists with a different user type.");
    }
  }
  ...
  if (factory.createdDecorator == nullptr) {
    throw ChimeraTK::logic_error(
        "ChimeraTK::ControlSystemAdapter: Decorator for TransferElement " +
        transferElement->getName() +
        " has been requested for an unknown user type: " +
