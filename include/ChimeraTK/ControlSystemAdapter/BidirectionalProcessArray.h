...
createBidirectionalSynchronizedProcessArray(
    std::size_t size, const ChimeraTK::RegisterPath &name = "",
 ...
 * ChimeraTK::ExperimentalFeatures in DeviceAccess).
 */
template <class T> class BidirectionalProcessArray : public ProcessArray<T> {
...
public:
  ...
  createBidirectionalSynchronizedProcessArray<>(
      ...
      const std::string &, T, std::size_t, ProcessVariableListener::SharedPtr,
  ...
  BidirectionalProcessArray(
      std::size_t uniqueId, const ChimeraTK::RegisterPath &name,
      const std::string &unit, const std::string &description,
      bool allowPersistentDataStorage,
  ...
  bool writeDestructively(ChimeraTK::VersionNumber versionNumber = {})
      override; /// @todo FIXME this function must be present in TransferElement
                /// already!

  void setPersistentDataStorage(
      boost::shared_ptr<PersistentDataStorage> storage) override;
...
BidirectionalProcessArray<T>::BidirectionalProcessArray(
    ...
    : ProcessArray<T>(ProcessArray<T>::SENDER_RECEIVER, name, unit, description,
      ...
      _receiver(receiver),
      _sender(
          boost::dynamic_pointer_cast<UnidirectionalProcessArray<T>>(sender)),
...
template <class T> void BidirectionalProcessArray<T>::doPreRead() {
  _receiver->preRead();
}

/*********************************************************************************************************************/

template <class T> void BidirectionalProcessArray<T>::doReadTransfer() {
  do {
    _receiver->read();
...
template <class T> bool BidirectionalProcessArray<T>::doReadTransferLatest() {
...
ChimeraTK::TransferFuture BidirectionalProcessArray<T>::doReadTransferAsync() {
  auto notificationQueue = ChimeraTK::detail::getFutureQueueFromTransferFuture(
      _receiver->readAsync());
  ...
  return {notificationQueue.template then<void>(continuation,
                                                std::launch::deferred),
          this};
}
...
template <class T> void BidirectionalProcessArray<T>::doPostRead() {
...
template <class T> void BidirectionalProcessArray<T>::doPreWrite() {}
...
template <class T> void BidirectionalProcessArray<T>::doPostWrite() {}
...
bool BidirectionalProcessArray<T>::writeDestructively(
  ...
  throw ChimeraTK::runtime_error(
      "This process variable must not be sent destructively because it is a "
...
void BidirectionalProcessArray<T>::setPersistentDataStorage(
  ...
  if (_allowPersistentDataStorage) {
    throw ChimeraTK::logic_error(
        "This device side of a process array must not be associated with a "
...
createBidirectionalSynchronizedProcessArray(
    ...
    ProcessVariableListener::SharedPtr sendNotificationListener2,
    const AccessModeFlags &flags) {
  auto senderReceiver1 = createSynchronizedProcessArray(
      size, name, unit, description, initialValue, numberOfBuffers, true, {},
      flags);
  auto senderReceiver2 = createSynchronizedProcessArray(
      size, name, unit, description, initialValue, numberOfBuffers, true, {},
      flags);
  ...
  typename boost::shared_ptr<BidirectionalProcessArray<T>> pv1 =
      boost::make_shared<BidirectionalProcessArray<T>>(
          uniqueId, name, unit, description, true, senderReceiver2.second,
          senderReceiver1.first, sendNotificationListener1, versionNumber,
          flags);
  typename boost::shared_ptr<BidirectionalProcessArray<T>> pv2 =
      boost::make_shared<BidirectionalProcessArray<T>>(
          uniqueId, name, unit, description, false, senderReceiver1.second,
          senderReceiver2.first, sendNotificationListener2, versionNumber,
          flags);
  pv1->_partner = pv2;
  pv2->_partner = pv1;
...
typename std::pair<typename ProcessArray<T>::SharedPtr,
                   typename ProcessArray<T>::SharedPtr>
createBidirectionalSynchronizedProcessArray(
    const std::vector<T> &initialValue, const ChimeraTK::RegisterPath &name,
    const std::string &unit, const std::string &description,
    std::size_t numberOfBuffers,
    ProcessVariableListener::SharedPtr sendNotificationListener1,
    ProcessVariableListener::SharedPtr sendNotificationListener2,
    const AccessModeFlags &flags) {
  auto senderReceiver1 = createSynchronizedProcessArray(
      initialValue, name, unit, description, numberOfBuffers, true, {}, flags);
  auto senderReceiver2 = createSynchronizedProcessArray(
      initialValue, name, unit, description, numberOfBuffers, true, {}, flags);
  // We create a default-constructed version number because we
  // want to be sure that initially both sides have the same time stamp and
