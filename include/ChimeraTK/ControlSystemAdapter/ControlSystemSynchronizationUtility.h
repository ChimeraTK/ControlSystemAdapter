...
namespace ChimeraTK {
  ...
  class ControlSystemSynchronizationUtility {
   public:
     ...
     */
    void addReceiveNotificationListener(ChimeraTK::RegisterPath const& processVariableName,
        ProcessVariableListener::SharedPtr receiveNotificationListener);
     ...
     * specified process variable, this method does nothing.
     */
     ...
     */
    void removeReceiveNotificationListener(ChimeraTK::RegisterPath const& processVariableName);
     ...
     */
    void waitForNotifications(long timeoutInMicroseconds, long checkIntervalInMicroseconds);
   ...
   private:
    ...
    /**
     * Map storing the receive listeners. The maps uses the raw pointer to the
     * process variable as the key and the corresponding receive listener as the
     * value. (Using the pointer instead of the name allows faster hashing).
     */
    std::unordered_map<ProcessVariable*, ProcessVariableListener::SharedPtr> _receiveNotificationListeners;

    // Disable copy construction and assignment.
    ControlSystemSynchronizationUtility(ControlSystemSynchronizationUtility const&);
    ControlSystemSynchronizationUtility& operator=(ControlSystemSynchronizationUtility const&);
  };
  ...
  template<class T>
  void ControlSystemSynchronizationUtility::receive(T processVariablesBegin, T processVariablesEnd) {
    for(T i = processVariablesBegin; i != processVariablesEnd; ++i) {
      if((*i)->readNonBlocking()) {
        auto listenerIterator = _receiveNotificationListeners.find(i->get()); // get the raw pointer from the shared
                                                                              // pointer which is iterated
  ...
  template<class T>
  void ControlSystemSynchronizationUtility::send(T processVariablesBegin, T processVariablesEnd) {
    for(T i = processVariablesBegin; i != processVariablesEnd; ++i) {
      (*i)->write();
    }
  }
