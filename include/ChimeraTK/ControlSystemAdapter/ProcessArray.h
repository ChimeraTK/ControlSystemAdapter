...
public:
   ...
   */
  virtual bool
  writeDestructively(ChimeraTK::VersionNumber versionNumber =
                         {}) = 0; /// @todo FIXME this function must be present
                                  /// in TransferElement already!
...
ProcessArray<T>::ProcessArray(InstanceType instanceType,
    ...
    : ChimeraTK::NDRegisterAccessor<T>(name, unit, description),
      _instanceType(instanceType), _flags(flags) {}

/*********************************************************************************************************************/

template <class T> ProcessArray<T>::~ProcessArray() {}
