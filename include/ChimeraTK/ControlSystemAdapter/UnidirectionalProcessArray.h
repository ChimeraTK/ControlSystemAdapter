...
extern std::atomic<bool>
    processArrayEnableThreadSafetyCheck; // std::atomic<bool> defaults to false
 ...
 */
template <class T> class UnidirectionalProcessArray : public ProcessArray<T> {
...
private:
  ...
  std::atomic<bool>
      threadSafetyCheckInitialised; // std::atomic<bool> defaults to false
};
...
UnidirectionalProcessArray<T>::UnidirectionalProcessArray(
    ...
    : ProcessArray<T>(instanceType, name, unit, description, flags),
      _vectorSize(initialValue.size()), _maySendDestructively(false),
  ...
  if (!this->isReadable()) {
    throw ChimeraTK::logic_error(
        "This constructor may only be used for a receiver process variable.");
  }
...
UnidirectionalProcessArray<T>::UnidirectionalProcessArray(
  ...
  if (!this->isWriteable()) {
    throw ChimeraTK::logic_error(
        "This constructor may only be used for a sender process variable.");
  }
  if (!receiver) {
    throw ChimeraTK::logic_error(
...
template <class T> void UnidirectionalProcessArray<T>::doReadTransfer() {
...
template <class T> bool UnidirectionalProcessArray<T>::doReadTransferLatest() {
...
ChimeraTK::TransferFuture UnidirectionalProcessArray<T>::doReadTransferAsync() {
  ...
  return TransferFuture(_sharedState._queue.template then<void>(
                            [this](Buffer &b) { std::swap(_localBuffer, b); },
                            std::launch::deferred),
                        this);
  /// @todo if wait_for_new_data is not set, make identical to
...
template <class T> void UnidirectionalProcessArray<T>::doPostRead() {
  ...
  assert(ChimeraTK::NDRegisterAccessor<T>::buffer_2D[0].size() ==
         _localBuffer._value.size());
...
void UnidirectionalProcessArray<T>::setPersistentDataStorage(
  ...
  if (sendInitialValue) {
    if (_persistentDataStorage->retrieveValue<T>(_persistentDataStorageID)
            .size() == ChimeraTK::NDRegisterAccessor<T>::buffer_2D[0].size()) {
...
bool UnidirectionalProcessArray<T>::writeInternal(
  ...
  if (ChimeraTK::NDRegisterAccessor<T>::buffer_2D[0].size() != _vectorSize) {
    throw ChimeraTK::logic_error(
        "Cannot run receive operation because the size of the vector belonging "
  ...
  if (newVersionNumber < _versionNumber) {
    throw ChimeraTK::logic_error(
        "The version number passed to write is less than or equal to the last "
        "version number used.");
  }
  ...
  if (_persistentDataStorage) {
    _persistentDataStorage->updateValue(
        _persistentDataStorageID,
        ChimeraTK::NDRegisterAccessor<T>::buffer_2D[0]);
  }
  ...
  assert(_localBuffer._value.size() ==
  ...
  bool dataNotLost =
      _sharedState._queue.push_overwrite(std::move(_localBuffer));
...
createSynchronizedProcessArray(
  ...
  auto receiver = boost::make_shared<UnidirectionalProcessArray<T>>(
      ...
      std::vector<T>(size, initialValue), numberOfBuffers, flags);
  auto sender = boost::make_shared<UnidirectionalProcessArray<T>>(
      ProcessArray<T>::SENDER, maySendDestructively, sendNotificationListener,
      receiver, flags);
