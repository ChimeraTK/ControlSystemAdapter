...
public:
   ...
   */
  typedef std::pair<ProcessVariable::SharedPtr, ProcessVariable::SharedPtr>
      ProcessVariableSharedPtrPair;
   ...
   * performance.
   */
  typedef std::map<ChimeraTK::RegisterPath, ProcessVariableSharedPtrPair>
      ProcessVariableMap;
  ...
  createBidirectionalProcessArray(
      ChimeraTK::RegisterPath const &processVariableName,
      const std::vector<T> &initialValue,
      const std::string &unit = ChimeraTK::TransferElement::unitNotSet,
  ...
  createProcessArrayDeviceToControlSystem(
      ...
      const std::vector<T> &initialValue,
      const std::string &unit = ChimeraTK::TransferElement::unitNotSet,
      const std::string &description = "", bool maySendDestructively = false,
  ...
  std::pair<typename ProcessArray<T>::SharedPtr,
            typename ProcessArray<T>::SharedPtr>
  createProcessArrayControlSystemToDevice(
      ChimeraTK::RegisterPath const &processVariableName,
      const std::vector<T> &initialValue,
      const std::string &unit = ChimeraTK::TransferElement::unitNotSet,
      const std::string &description = "", bool maySendDestructively = false,
  ...
  std::pair<typename ProcessArray<T>::SharedPtr,
            typename ProcessArray<T>::SharedPtr>
  getProcessArray(ChimeraTK::RegisterPath const &processVariableName) const;
  ...
  getProcessVariable(ChimeraTK::RegisterPath const &processVariableName) const;
...
private:
   ...
   * variables.
   */
  class ControlSystemSendNotificationListenerImpl
   ...
   */
  boost::lockfree::queue<DeviceSendNotificationListenerImpl *>
      _controlSystemNotificationQueue;
   ...
   * of this queue if a process variable is added.
   */
  boost::lockfree::queue<ControlSystemSendNotificationListenerImpl *>
      _deviceNotificationQueue;
};
...
std::pair<typename ProcessArray<T>::SharedPtr,
          typename ProcessArray<T>::SharedPtr>
PVManager::createBidirectionalProcessArray(
    ChimeraTK::RegisterPath const &processVariableName,
  ...
  if (_processVariables.find(processVariableName) != _processVariables.end()) {
    throw ChimeraTK::logic_error("Process variable with name " +
                                 processVariableName + " already exists.");
  }
  ...
  typename std::pair<typename ProcessArray<T>::SharedPtr,
                     typename ProcessArray<T>::SharedPtr>
      processVariables = createBidirectionalSynchronizedProcessArray<T>(
          initialValue, processVariableName, unit, description, numberOfBuffers,
          sendNotificationListener1, sendNotificationListener2);

  _processVariables.insert(std::make_pair(
      processVariableName,
...
std::pair<typename ProcessArray<T>::SharedPtr,
          typename ProcessArray<T>::SharedPtr>
PVManager::createProcessArrayDeviceToControlSystem(
    ChimeraTK::RegisterPath const &processVariableName,
    const std::vector<T> &initialValue, const std::string &unit,
    const std::string &description, bool maySendDestructively,
  ...
  typename std::pair<typename ProcessArray<T>::SharedPtr,
                     typename ProcessArray<T>::SharedPtr>
      processVariables = createSynchronizedProcessArray<T>(
          initialValue, processVariableName, unit, description, numberOfBuffers,
...
std::pair<typename ProcessArray<T>::SharedPtr,
          typename ProcessArray<T>::SharedPtr>
PVManager::createProcessArrayControlSystemToDevice(
    ChimeraTK::RegisterPath const &processVariableName,
    const std::vector<T> &initialValue, const std::string &unit,
    const std::string &description, bool maySendDestructively,
    std::size_t numberOfBuffers, const AccessModeFlags &flags) {
...
std::pair<typename ProcessArray<T>::SharedPtr,
          typename ProcessArray<T>::SharedPtr>
PVManager::getProcessArray(
    ChimeraTK::RegisterPath const &processVariableName) const {
  ProcessVariableSharedPtrPair processVariable =
      getProcessVariable(processVariableName);
  typename ProcessArray<T>::SharedPtr csPV =
      boost::dynamic_pointer_cast<ProcessArray<T>, ProcessVariable>(
          processVariable.first);
  typename ProcessArray<T>::SharedPtr devPV =
      boost::dynamic_pointer_cast<ProcessArray<T>, ProcessVariable>(
          processVariable.second);
  if (!csPV || !devPV) {
    throw ChimeraTK::logic_error(
        "PVManager::getProcessArray() called for variable '" +
        processVariableName + "' with type " + typeid(T).name() +
        " which is not the original type " +
        processVariable.first->getValueType().name() +
